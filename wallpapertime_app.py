...
from PyQt5.QtWidgets import QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, \
    QPushButton, QTableWidget, QTableWidgetItem, QHeaderView, QCheckBox, QSystemTrayIcon, QMenu, QFileDialog, \
    QMessageBox
...
class WallpaperTimeApp(QMainWindow):
    def __init__(self):
        ...
        self.checkbox_update_thread = threading.Thread(target=self.continuously_update_checkbox_states)
        self.checkbox_update_thread.daemon = True  # Allow the thread to exit when the main program exits
        ...
        self.interval_table.setHorizontalHeaderLabels(["Start Time", "End Time", "Wallpaper Path"])
        ...
        self.minimize_button.setEnabled(True)  # Set the initial enabled state 

        self.layout = QVBoxLayout(self.central_widget)
        self.layout.addWidget(self.interval_table)
        ...
        self.start_time_input.setStyleSheet("background-color: rgb(105, 105, 105); color: white;")
        self.end_time_input.setStyleSheet("background-color: rgb(105, 105, 105); color: white;")
        self.wallpaper_path_input.setStyleSheet("background-color: rgb(105, 105, 105); color: white;")
        self.wallpaper_path_button.setStyleSheet("background-color: rgb(105, 105, 105); color: white;")
        self.add_button.setStyleSheet("background-color: rgb(105, 105, 105); color: white;")
        self.edit_button.setStyleSheet("background-color: rgb(105, 105, 105); color: white;")
        self.delete_button.setStyleSheet("background-color: rgb(105, 105, 105); color: white;")
        self.autostart_checkbox.setStyleSheet("color: white;")
        self.start_minimized_checkbox.setStyleSheet("color: white;")
        self.minimize_button.setStyleSheet("background-color: rgb(105, 105, 105); color: white;")
        header = self.interval_table.horizontalHeader()

        # Set the background color for the header
        header.setStyleSheet("QHeaderView::section { background-color: rgb(60, 60, 60); color: white; }")

    def continuously_update_checkbox_states(self):
        while True:
            self.load_settings()
    ...
    def save_settings(self):
        ...
        for i, (start_time, end_time, wallpaper_path) in enumerate(self.intervals):
            config.set("Intervals", f"interval{i}", f"{start_time},{end_time},{wallpaper_path}")
    ...
    def add_interval(self):
        ...
        if not start_time or not end_time or not wallpaper_path:
            QMessageBox.critical(self, "Error", "Please fill in all fields before adding an interval.")
            return

        if not self.validate_time_format(start_time) or not self.validate_time_format(end_time):
            QMessageBox.critical(self, "Error", "Invalid time format. Please use HH:MM format.")
            return
    ...
    def edit_interval(self):
        ...
        if selected_row != -1:
            ...
            if not start_time or not end_time or not wallpaper_path:
                QMessageBox.critical(self, "Error", "Please fill in all fields before editing the interval.")
                return
    ...
    def create_autostart_entry(self):
        ...
        try:
            subprocess.run(["reg", "add", key_path, "/v", key_name, "/d", value, "/f"], check=True)
    ...
    def remove_autostart_entry(self):
        ...
        try:
            subprocess.run(["reg", "delete", key_path, "/v", key_name, "/f"], check=True)
        except subprocess.CalledProcessError as e:
    ...
    def update_wallpaper(self):
        ...
        if wallpaper_path:
            ...
            print("No wallpaper set for the current time.")

    def set_wallpaper(self, path):
        # Check if the file exists
        if not os.path.exists(path):
            print(f"Error: The wallpaper file '{path}' does not exist.")
            return
    ...
    def select_wallpaper(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, "Select Wallpaper", "",
                                                   "Images (*.jpg *.png *.jpeg);;All Files (*)", options=options)
        if file_path:
...
if __name__ == "__main__":
    ...
    if not os.path.exists("icon.png"):
        print("The icon.png file is missing. Please place it in the same directory as the script.")
        sys.exit(1)
